...
export default function PaymentModal({ isOpen, onClose, onSubmit, loading: externalLoading }: PaymentModalProps) {
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in text-start">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl animate-scale-up">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="text-xl font-semibold text-slate-800">{ARABIC.suppliers.recordPayment}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex justify-end gap-3">
            ...
            >
              {isLoading ? ARABIC.common.saving : ARABIC.suppliers.recordPayment}
            </button>
          </div>
        </form>
      </div>
