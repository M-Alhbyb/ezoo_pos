    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl animate-scale-up">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="text-xl font-semibold text-slate-800">{ARABIC.projects.createProject}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          {error && (
            <div className="mb-6 p-3 bg-rose-50 text-rose-700 rounded-xl text-sm border border-rose-200 flex items-center">
              <svg className="w-4 h-4 me-2" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v4a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"></path></svg>
              {error}
            </div>
          )}
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">{ARABIC.projects.projectName}</label>
              <input
                type="text"
                required
                value={name}
                ...
                placeholder={ARABIC.projects.projectNamePlaceholder || 'مثال: تجميع حاسب آلي مخصص'}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium text-slate-800"
                placeholder={ARABIC.projects.leaveBlankCompute || 'اتركه فارغاً للحساب من المنتجات'}
              />
            </div>
          </div>
          ...
          <div className="mt-8 flex justify-end gap-3">
            ...
            >
              {loading ? ARABIC.common.saving : ARABIC.projects.createProject}
            </button>
          </div>
        </form>
      </div>
