...
export default function SystemSettingsManager() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div key={setting.id} className="glass p-6 rounded-2xl border border-slate-200/50 hover:border-primary/20 transition-all group">
            ...
            {setting.key === "vat_enabled" ? (
              <div className="flex items-center space-x-3">
                ...
                <span className="text-sm font-medium text-slate-600">{setting.value === "true" ? "Enabled" : "Disabled"}</span>
              </div>
      ...
      {settings.length === 0 && (
        <div className="py-12 text-center text-slate-400 italic">No settings found.</div>
      )}
    </div>
  );
}
