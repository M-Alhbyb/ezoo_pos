...
export default function PaymentMethodManager() {
  ...
  if (loading && methods.length === 0) {
    return <div className="p-8 text-center text-slate-500">Loading methods...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="glass overflow-hidden rounded-2xl border border-slate-200/50">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50 border-b border-slate-200">
              <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Method Name</th>
              ...
              <tr className="bg-blue-50/30">
                <td className="px-6 py-4" colSpan={2}>
                  <form onSubmit={handleAdd} className="flex space-x-2">
                    ...
                    />
                    <button type="submit" className="px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700">Save</button>
                    <button type="button" onClick={() => setIsAdding(false)} className="px-3 py-1.5 bg-slate-200 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-300">Cancel</button>
                  </form>
                </td>
                <td></td>
              </tr>
        ...
        {methods.length === 0 && !isAdding && (
          <div className="py-12 text-center text-slate-400 italic">No payment methods found. Add one to get started.</div>
        )}
      </div>
    </div>
  );
