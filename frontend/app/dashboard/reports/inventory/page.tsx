...
export default function InventoryReportPage() {
  ...
  const columns: Column<InventoryMovementData>[] = [
    { header: "Reason", accessor: "reason", className: "font-bold text-slate-800 capitalize" },
    { 
      header: "Total Delta", 
    ...
    { 
        ...
        render: (item) => (
            <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${
                ...
                {item.total_delta >= 0 ? "Restock/Adjustment" : "Sale/Reduction"}
            </span>
        ) 
    },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-4xl font-extrabold font-heading text-slate-900 tracking-tight">Stock Logistics</h1>
          <p className="text-slate-500 mt-2 font-medium">Tracking all inventory movements, adjustments, and restocking events.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="glass-card p-8 hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col justify-center">
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Movements</p>
